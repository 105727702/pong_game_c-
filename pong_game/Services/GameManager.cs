...
namespace PongGame.Services
{
    ...
    public class GameManager
    {
        ...
        public List<Wall> GenerateWalls(int numWalls, int minDistance)
        {
            ...
            float speedMultiplier = 1.0f + (numWalls - 4) * 0.1f; // Base 4 walls, +10% speed per additional wall
            float playableStartX = 60f;
            float playableEndX = 1140f; // Căn chỉnh với vùng hiển thị thực tế
        ...
        public bool CheckBallOutOfBounds(SoundManager soundManager, PotionEffectManager potionEffectManager = null)
        {
            ...
            if (_ball.X < 1) 
            {
                ...
                if (_scoreboard.RightScore >= 1)
                {
                    HandleGameOver(2, soundManager);
                    return true;
                }
            }
            // Right boundary - dựa trên vùng hiển thị thực tế (sau x=1200)
